...
def extract_features(imgs, cspace='RGB', spatial_size=(32, 32),
    ...
    for file in imgs:
        ...
        feature_image = cv2.resize(image,None, fx=1/2, fy=1/2, interpolation = cv2.INTER_LINEAR)    
...
def extract_features_augmented(imgs, cspace='RGB', spatial_size=(32, 32),
                        hist_bins=32, hist_range=(0, 256), orient = 9, 
                        pix_per_cell = 8, cell_per_block = 2):
    """Computes feature vector of a list of images (imgs is list of filenames)
    feature vector is concatenation of spatial, color and HOG features"""
...
#        feature_image = cv2.equalizeHist(feature_image)
        ...
        if augment_b == True:
            for i in range(8):      
#                zoomscale = np.random.rand(1)*0.5+1 # up to 1.5x zoom
#                offsetscale_x = (zoomscale-1)*(np.random.rand(1)-0.5) #max shift limited by scale            
#                offsetscale_y = (zoomscale-1)*(np.random.rand(1)-0.5) 
                
                scale_max = 0.25
                zoomscale = np.random.rand(1)*scale_max+1 # up to 1.5x zoom
                offsetscale_x = (zoomscale-1)*(np.random.rand(1)-scale_max) #max shift limited by scale            
                offsetscale_y = (zoomscale-1)*(np.random.rand(1)-scale_max) 
                
#                
                
                feature_image_aug = img_zoom(feature_image, zoomscale,[offsetscale_x,offsetscale_y])
            
                # Apply bin_spatial() to get spatial color features
                spatial_features = bin_spatial(feature_image_aug, size=spatial_size)
                # Apply color_hist() also with a color space option now
                hist_features = color_hist(feature_image_aug, nbins=hist_bins, bins_range=hist_range)
                
                # Apply hog_features:        
                # testing only using Y channel, a 1% drop in accuracy but a huge speed up
                hog1 = get_hog_features(feature_image_aug[:,:,0], orient, pix_per_cell, cell_per_block, vis=False,feature_vec=False)
#                hog2 = get_hog_features(feature_image_aug[:,:,1], orient, pix_per_cell, cell_per_block, vis=False,feature_vec=False)
#                hog3 = get_hog_features(feature_image_aug[:,:,2], orient, pix_per_cell, cell_per_block, vis=False,feature_vec=False)
...
def draw_labeled_bboxes(img, labels, scale = 1):
    """Draw bounding boxes onto img, labels has same dimension as img.shape/scale with 
    ...
    for car_number in range(1, labels[1]+1):
        ...
        if np.abs(bbox[0][0]-bbox[1][0]) > 35 and np.abs(bbox[0][1]-bbox[1][1]) > 35:
...
def img_zoom(img, zoomfactor=1.2, offsetfactor = [0,0]):
    
    '''Zoom in on training image for use in data augmentation'''
    
    h,w = img.shape[0:2]
    M = cv2.getRotationMatrix2D( (w/2,h/2), 0, zoomfactor) 
    M[0,2]+=offsetfactor[0]*w
    M[1,2]+=offsetfactor[1]*h
